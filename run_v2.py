...
class Apc40StateMachine:
    ...
    def __init__(
        ...
        try:
            ...
            try:
                ...
                print(f"[OSC] Using show_bridge OSC at {sb_host}:{sb_port} for state broadcasts")
            except Exception as sb_e:
                print(f"[OSC] WARNING: could not initialize show_bridge OSC: {sb_e}")
                sb_osc_client = None
        ...
        if state_output_cfg is not None:
            ...
            try:
                ...
                print(f"[STATE-OUTPUT] WARNING: failed to load state-machine output config: {e}")
                self.state_output_cfg = {}
    ...
    def on_clip_connect(self, osc_layer_index: int, osc_clip_index: int) -> None:
        ...
        # --- Resolve LayerInfo + ClipInfo for debug + clip-name broadcast ---
        layer_info = None
        ...
        # --- NEW: Broadcast updated state + clip name ---
        # Broadcast all roles / names again so TD sees the new truth table
        if hasattr(self, "broadcast_full_state"):
            try:
                self.broadcast_full_state()
            except Exception as e:
                print(f"[OSC] WARNING: broadcast_full_state failed after clip connect: {e}")

        # Broadcast this layer's clip name explicitly, if helper exists
        if hasattr(self, "_broadcast_clip_name") and layer_info is not None:
            try:
                self._broadcast_clip_name(layer_info, clip_info)
            except Exception as e:
                print(f"[OSC] WARNING: _broadcast_clip_name failed: {e}")
    ...
    def on_transport_beat(self, beat_kind: str) -> None:
        ...
        print(f"[AUTOPILOT] Received beat4, beats_remaining={self.autopilot_beats_remaining}, global_autopilot={self.global_autopilot}")

        if self.autopilot_beats_remaining > 0:
            self.autopilot_beats_remaining -= 1

        if self.autopilot_beats_remaining <= 0:
            print("[AUTOPILOT] Threshold reached, running cycle")
    ...
    def broadcast_full_state(self) -> None:
        """
        Broadcast a full snapshot of the current state-machine:

          - Global state (global intensity, autopilot, etc.)
          - All group-level booleans (playing, effects, transforms, etc.)
          - All group-level numeric values (intensity, opacity)

        Uses the YAML-driven state_output_cfg where possible.
        """
        if not self.state_output_cfg:
            print("[OSC] broadcast_full_state: no state_output_cfg, skipping.")
            return

        if self.broadcast_osc_client is None:
            print("[OSC] broadcast_full_state: no broadcast_osc_client, skipping.")
            return

        print("[OSC] BROADCAST: full state snapshot")

        # --- Global state ---
        try:
            self._broadcast_state_update(None, "global_intensity_scale", getattr(self, "global_intensity_scale", 1.0))
        except Exception as e:
            print(f"[OSC] WARNING broadcasting global_intensity_scale: {e}")

        try:
            self._broadcast_state_update(None, "autopilot", getattr(self, "global_autopilot", False))
        except Exception as e:
            print(f"[OSC] WARNING broadcasting global autopilot: {e}")

        # --- Group-level state ---
        for group_idx, g in enumerate(self.groups):
            # boolean props (e.g. playing, effects, transforms, dynamic_masks, color)
            for prop in getattr(self, "BOOL_PROPS", []):
                try:
                    val = getattr(g, prop)
                ...
                try:
                    ...
                    print(f"[OSC] WARNING broadcasting {prop} for group {group_idx+1}: {e}")

            # numeric props (intensity, opacity) if present
            for prop in ("intensity", "opacity"):
                if hasattr(g, prop):
                    try:
                        val = getattr(g, prop)
                        self._broadcast_state_update(group_idx, prop, val)
    ...
    def initialize_state_from_composition(self) -> None:
        """
        Sync our internal layer_states + group booleans from the attached
        composition, then broadcast a full state snapshot via OSC.
        """
        if self.composition is None:
            print("[INIT] No composition attached; skipping sync.")
            return

        print("[INIT] Syncing with composition...")

        # Pass 1 — per-layer ON/OFF based on active column
        for group in self.composition.groups:
            for layer in group.layers:
        ...
        # Pass 2 — aggregate per-group boolean props
        ...
        # NEW: broadcast a full state snapshot
        try:
            self.broadcast_full_state()
        except Exception as e:
            print(f"[OSC] WARNING: broadcast_full_state during init failed: {e}")
 # ---- State output broadcast helper ----
    def _broadcast_clip_name(self, layer_info, clip_info) -> None:
        """
        Broadcast the clip name for a given layer.

        Expected YAML shape (optional):

          outputs:
            layer:
              clip_name: "/state/layer/{layer}/clip_name"

        Available format fields:
          - {layer}       : 1-based global layer index
          - {layer_name}  : layer name string
          - {clip}        : 1-based clip index (column)
          - {clip_name}   : clip name string (not substituted into address, but passed as arg)
        """
        if self.broadcast_osc_client is None:
            print("[OSC] _broadcast_clip_name: no broadcast_osc_client, skipping.")
            return

        name = clip_info.name if clip_info is not None else ""
        layer_idx_1based = layer_info.global_index + 1
        clip_idx_1based = getattr(clip_info, "column_index", None) or 0

        outputs = self.state_output_cfg.get("outputs", {})
        layer_cfg = outputs.get("layer", {})
        tmpl = layer_cfg.get("clip_name")

        if tmpl:
            addr = tmpl.format(
                layer=layer_idx_1based,
                layer_name=layer_info.name,
                clip=clip_idx_1based,
                clip_name=name,
            )
        else:
            # Sensible default if YAML doesn't define anything
            addr = f"/state/layer/{layer_idx_1based}/clip_name"

        print(f"[OSC] BROADCAST {addr} {name!r}")
        try:
            self.broadcast_osc_client.send_message(addr, name)
        except Exception as e:
            print(f"[OSC] ERROR broadcasting clip_name {addr} {name!r}: {e}")
    ...
    def _broadcast_state_update(
        ...
        """
        Publish a state-machine update via OSC using the state_output_cfg
        loaded from settings/state_machine_mappings.yaml.

        Broadcasts use the show_bridge OSC client (broadcast_osc_client),
        not the Resolume control client.
        """
        if not self.state_output_cfg:
            return

        if self.broadcast_osc_client is None:
            # Don't spam too hard, but it's useful while wiring things up
            print(
                f"[OSC] (no show_bridge client) WOULD BROADCAST state "
                f"group={group_idx}, prop={prop}, value={value!r}"
            )
            return

        # Normalize the value type
        ...
        outputs = self.state_output_cfg.get("outputs", {})
        group_cfg = outputs.get("group", {})
        global_cfg = outputs.get("global", {})

        addr = None

        if group_idx is not None:
            role_map = group_cfg.get(kind, {})
            tmpl = role_map.get(prop)
            if tmpl:
                addr = tmpl.format(group=group_idx + 1)
        else:
            role_map = global_cfg.get(kind, {})
            addr = role_map.get(prop)

        if not addr:
            return

        print(f"[OSC] BROADCAST {addr} {v!r}")
        try:
            self.broadcast_osc_client.send_message(addr, v)
        except Exception as e:
            print(f"[OSC] ERROR broadcasting {addr} {v!r}: {e}")
    def _get_role_clip_name(self, group_idx: int, prop: str) -> str:
        """
        Best-effort guess of the human-readable name for a given role (prop)
        inside a group, based on layer_states + composition mapping.
        """
        if self.composition is None:
            return ""
        ...
        def _layer_clip_name(layer: LayerInfo) -> str:
            st = self.layer_states.get(layer.global_index)
            if not st:
                return ""
            col = st.current_clip_index or 1
            for c in layer.clips:
                if c.column_index == col:
                    return c.name or ""
            return ""

        # Prefer a playing layer first
        for layer in role_layers:
            st = self.layer_states.get(layer.global_index)
            if st and st.playing:
                name = _layer_clip_name(layer)
                if name:
                    return name

        # Fallback: any layer with a resolvable name
        for layer in role_layers:
            name = _layer_clip_name(layer)
            if name:
                return name
    ...
    def _broadcast_role_enabled_and_name(self, group_idx: int, prop: str) -> None:
        """
        Broadcast both the enabled flag and the human readable name for
        a given role (prop) inside a group.

        Address scheme:

          /state/group/<group>/<prop>/enabled   1|0
          /state/group/<group>/<prop>/name      "Snow 2"
        """
        if self.broadcast_osc_client is None:
            return

        g = self.groups[group_idx]
        enabled = bool(getattr(g, prop, False))
        name = self._get_role_clip_name(group_idx, prop)

        group_num = group_idx + 1

        # enabled
        addr_enabled = f"/state/group/{group_num}/{prop}/enabled"
        val_enabled = 1.0 if enabled else 0.0
        print(f"[OSC] BROADCAST {addr_enabled} {val_enabled!r}")
        try:
            self.broadcast_osc_client.send_message(addr_enabled, val_enabled)
        except Exception as e:
            print(f"[OSC] ERROR broadcasting {addr_enabled} {val_enabled!r}: {e}")

        # name
        addr_name = f"/state/group/{group_num}/{prop}/name"
        print(f"[OSC] BROADCAST {addr_name} {name!r}")
        try:
            self.broadcast_osc_client.send_message(addr_name, name)
        except Exception as e:
            print(f"[OSC] ERROR broadcasting {addr_name} {name!r}: {e}")

    def _broadcast_group_roles(self, group_idx: int) -> None:
        """Broadcast all role states (enabled + name) for a single group."""
        for prop in self.BOOL_PROPS:
            self._broadcast_role_enabled_and_name(group_idx, prop)

    def broadcast_full_state(self) -> None:
        """
        Broadcast a full snapshot of the current state-machine:

          - Global state (intensity, autopilot, etc.) via the YAML mapping
            in state_output_cfg (if present).
          - For each group and each role (playing, effects, transforms,
            dynamic_masks, color), broadcast:

                /state/group/<group>/<prop>/enabled
                /state/group/<group>/<prop>/name
        """
        if self.broadcast_osc_client is None:
            print("[OSC] broadcast_full_state: no broadcast_osc_client, skipping.")
            return
        ...
        # Keep the old YAML-driven outputs alive for global stuff
        try:
            self._broadcast_state_update(
                None,
                "global_intensity_scale",
                getattr(self, "global_intensity_scale", 1.0),
            )
        except Exception as e:
            print(f"[OSC] WARNING broadcasting global_intensity_scale: {e}")
        ...
        try:
            self._broadcast_state_update(
                None,
                "autopilot",
                getattr(self, "global_autopilot", False),
            )
        except Exception as e:
            print(f"[OSC] WARNING broadcasting global_autopilot: {e}")

        # New: group roles as enabled + name
        for group_idx in range(len(self.groups)):
            self._broadcast_group_roles(group_idx)
    def _broadcast_clip_name(self, layer_info: LayerInfo, clip_info: Optional[ClipInfo]) -> None:
        """
        Broadcast the currently selected clip name for a given layer.

        Address:

          /state/layer/<layer>/clip_name   "Some Clip"
        """
        if self.broadcast_osc_client is None:
            return

        layer_idx_1based = layer_info.global_index + 1
        clip_name = clip_info.name if clip_info is not None else ""

        addr = f"/state/layer/{layer_idx_1based}/clip_name"
        print(f"[OSC] BROADCAST {addr} {clip_name!r}")
        try:
            self.broadcast_osc_client.send_message(addr, clip_name)
        except Exception as e:
            print(f"[OSC] ERROR broadcasting {addr} {clip_name!r}: {e}")
    ...
    def set_global_autopilot(self, value: bool):
        ...
        # Broadcast global autopilot state
        try:
            self._broadcast_state_update(None, "autopilot", value)
        except Exception:
            pass
...
def start_transport_osc_listener(sm: Apc40StateMachine, host: str = "127.0.0.1", port: int = 7001):
    ...
    like /czechb/transport/beat4/trigger.
    """
    ...
    def make_handler(beat_kind: str):
        def handler(address: str, *args):
            ...
            if v > 0.5:
                print(f"[OSC-IN] Beat {beat_kind} from {address} value={v}")
...
def main(argv: Optional[List[str]] = None):
    ...
    try:
        ...
        try:
            ...
            print("[STATE] Loaded state_machine_mappings.yaml for OSC broadcast.")
        except FileNotFoundError:
            print("[STATE] No settings/state_machine_mappings.yaml found; state broadcast mapping disabled.")
            state_output_cfg = {}
        except Exception as cfg_e:
            print(f"[STATE] WARNING: could not load state_machine_mappings.yaml: {cfg_e}")
            state_output_cfg = {}
